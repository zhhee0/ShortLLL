...
(
    `id`          bigint(20) NOT NULL AUTO_INCREMENT COMMENT 'ID',
    `title`       varchar(64) DEFAULT NULL COMMENT '鏁呬簨鏍囬',
    `content`     text COMMENT '鏁呬簨鍐呭',
    `username`    varchar(256) DEFAULT NULL COMMENT '浣滆€呯敤鎴峰悕',
    `like_count`  int(11) DEFAULT 0 COMMENT 'like count',
    `create_time` datetime DEFAULT NULL COMMENT '鍒涘缓鏃堕棿',
    `update_time` datetime DEFAULT NULL COMMENT '淇敼鏃堕棿',
    `del_flag`    tinyint(1) DEFAULT NULL COMMENT '鍒犻櫎鏍囪瘑 0锛氭湭鍒犻櫎 1锛氬凡鍒犻櫎',
    PRIMARY KEY (`id`),
    KEY `idx_story_create_time` (`create_time`) USING BTREE,
    KEY `idx_story_username` (`username`) USING BTREE,
    KEY `idx_story_like_count` (`like_count`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
(
    ...
    `del_flag`    tinyint(1) DEFAULT NULL COMMENT 'delete flag',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_story_username` (`story_id`, `username`) USING BTREE,
    KEY `idx_story_like_story_id` (`story_id`) USING BTREE,
    KEY `idx_story_like_username` (`username`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
