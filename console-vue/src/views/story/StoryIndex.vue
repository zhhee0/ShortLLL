        ...
        <div v-if="stories.length" class="story-list">
          <div v-for="story in stories" :key="story.id" class="story-card">
            <div class="story-actions-right">
              <el-button
                v-if="canPublish && (isAdmin || story.username === currentUsername)"
              ...
              <button class="like-btn" :class="{ liked: story.liked }" @click="handleLike(story)">
                👍 <span class="like-count">{{ story.likeCount || 0 }}</span>
              </button>
            </div>
...
const storySortValue = (story) => {
  const likeCount = Number.isFinite(story?.likeCount) ? story.likeCount : 0
  const timeValue = parseStoryTime(story?.createTime)
  return [likeCount, timeValue]
}

const sortStories = () => {
  stories.value = [...stories.value].sort((a, b) => {
    const [likeA, timeA] = storySortValue(a)
    const [likeB, timeB] = storySortValue(b)
    if (likeA !== likeB) {
      return likeB - likeA
    }
    return timeB - timeA
...
const handleLike = async (story) => {
  ...
  try {
    ...
    if (res?.data?.code !== '0') {
      ElMessage.error(res?.data?.message || '点赞失败，请稍后再试')
      return
    }
    ...
    const nextLiked = data?.liked ?? !story.liked
    const delta = nextLiked ? 1 : -1
    story.liked = nextLiked
    story.likeCount = Math.max(0, (story.likeCount || 0) + delta)
    sortStories()
  } catch (error) {
...
.like-btn {
  ...
  padding: 3px 12px;
  border-radius: 16px;
  border: 1px solid rgba(111, 251, 255, 0.35);
  background: rgba(111, 251, 255, 0.1);
  color: #7ef4ff;
  cursor: pointer;
  font-size: 13px;
  font-family: inherit;
...
.like-btn.liked {
  background: rgba(255, 110, 165, 0.2);
  border-color: rgba(255, 110, 165, 0.6);
  color: #ff9ac2;
}
...
.like-btn:hover {
  background: rgba(111, 251, 255, 0.22);
  border-color: rgba(111, 251, 255, 0.6);
